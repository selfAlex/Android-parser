...
def parse_shop():
    ...
    def get_pages_count():
        ...
        soup = BeautifulSoup(html.text, 'html.parser',
                             parse_only=SoupStrainer('div', {'class': 'bx_catalog_item double'}))
    ...
    for page in range(1, get_pages_count() + 1):
        ...
        soup = BeautifulSoup(html.text, 'html.parser', parse_only=SoupStrainer('div', {
            'class': 'bx_catalog_item_container gtm-impression-product'}))
        ...
        for item in items:
            data.append({
                ...
                'image_url': 'https:' +
                             item.find('div', class_='bx_catalog_item_container gtm-impression-product').find('figure',
                                                                                                              class_='item_image_container').find(
                                 'a')['style'][23:-2],
                ...
                'url': 'https://shop.kz' +
                       item.find('div', class_='bx_catalog_item_container gtm-impression-product').find('div',
                                                                                                        class_='bx-catalog-middle-part').find
...
def parse_forcecom():
    ...
    for page in range(1, get_pages_count() + 1):
        html = get_html(url, params={'PAGEN_1': page})
        soup = BeautifulSoup(html.text, 'html.parser',
                             parse_only=SoupStrainer('div', {'class': 'list_item_wrapp item_wrap item'}))
        ...
        for item in items:
            ...
            try:
                data[-1]['cost'] = \
                    int(item.find('table').find('tr', class_='').find('td',
                                                                      class_='information_wrapp main_item_wrapper').find(
