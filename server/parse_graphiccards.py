...
def parse_shop():
    data = []
    url = 'https://shop.kz/videokarty/filter/karaganda-is-v_nalichii-or-ojidaem-or-dostavim/apply/'
...
def parse_forcecom():
    data = []
    url = 'https://forcecom.kz/catalog/graphics-cards/'
        ...
        for item in items:
            data.append({
                ...
                'title': item.find('table').find('tr', class_='').find('td', class_='description_wrapp').find('div',
                    ...
                    'div', class_='item-title').find('a').find('span').get_text()[11:],
...
def parse_tomas():
    data = []
    url = 'https://tomas.kz/t/videokarty-1255/'
